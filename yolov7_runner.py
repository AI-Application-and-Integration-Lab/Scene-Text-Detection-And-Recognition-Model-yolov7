...
class Yolov7Runner:
    ...
    def run(self):
        with torch.no_grad():
            all_labels = self.detect()
        
        for labels in all_labels:
            all_labels[labels] = [line.strip().split(" ") \
                                    for line in all_labels[labels]]
        return all_labels
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        all_labels[p.name].append("%s %s" %(names[int(c)], output))
